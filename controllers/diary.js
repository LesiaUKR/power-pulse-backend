...
const addProduct = async (req, res) => {
  ...
  let originalProduct = await DiaryProducts.findOne({
    owner,
    date,
    productId,
  }).populate("productId", "category title groupBloodNotAllowed");
  if (!originalProduct) {
  ...
  const { category, title, groupBloodNotAllowed } =
    originalProduct.productId || originalProduct;
...
const deleteProduct = async (req, res) => {
  ...
  const productToDelete = await DiaryProducts.findOneAndDelete({
    _id,
    date,
    owner,
  });
...
const addExercise = async (req, res) => {
  ...
  const date = formatDate(new Date());
  let originalExercise = await DiaryExercises.findOne({
    owner,
    date,
    exerciseId,
  }).populate("exerciseId", "bodyPart equipment name target");
  ...
  const { bodyPart, equipment, name, target } =
    originalExercise.exerciseId || originalExercise;
...
const deleteExercise = async (req, res) => {
  ...
  const exerciseToDelete = await DiaryExercises.findOneAndDelete({
    _id,
    date,
    owner,
  });
...
const getDiary = async (req, res) => {
  ...
  const eatenProducts = await DiaryProducts.find({ owner, date }).populate(
    "productId",
    "category title"
  );
  const doneExercises = await DiaryExercises.find({ owner, date }).populate(
    "exerciseId",
    "bodyPart equipment name target"
  );

  const consumedCalories = eatenProducts
    .map((product) => product.calories)
    .reduce((prev, val) => (prev += val), 0);
  const burnedCalories = doneExercises
    .map((exercise) => exercise.burnedCalories)
    .reduce((prev, val) => (prev += val), 0);
  const sportTime = doneExercises
    .map((exercise) => exercise.time)
    .reduce((prev, val) => (prev += val), 0);
