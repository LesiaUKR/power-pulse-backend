...
const dailyMetrics = async (req, res, next) => {
  ...
  const { name, height, currentWeight, birthday, sex, levelActivity } =
    req.body;
  const dci = Math.floor(
    calculateBMR(currentWeight, height, birthday, levelActivity, sex)
  );
  if (name) {
    delete req.body.name;
    updBodyParams = {
      name,
      bodyParams: { ...req.body, dailyIntakeCalories: dci },
    };
  } else
    updBodyParams = { bodyParams: { ...req.body, dailyIntakeCalories: dci } };
  try {
